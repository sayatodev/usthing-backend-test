...
export class ScraperService {
  private readonly HKUST_URL = 'https://bmundergrad.hkust.edu.hk/announcement';
  ...
  private async unsafeScrapeHKUST(page: Page): Promise<string[]> {
    ...
    return titles
      .filter((t) => keywordsLower.some((k) => t.toLowerCase().includes(k)))
      .map((t) => `${t} [UST]`);
  }
  ...
  async scrapeCompetitions(): Promise<{
    ...
    try {
      ...
      const unique: { idx: number; original: string; normalized: string }[] =
        [];
      combined.forEach((text, idx) => {
        ...
        if (!dup) unique.push({ idx, original: text, normalized: norm });
      });
      return {
        status: 'success',
